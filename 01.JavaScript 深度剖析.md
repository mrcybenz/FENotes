...
## TypeScript 的类型声明
> 对于 ts 项目中用到的三方模块，如果不是用 ts 编写，则需要引入相应的类型声明文件
...
> 标记清除：直接从全局对象标记对象是否可达，可达则标记为活动对象；
>> 优点：可以回收循环引用的对象
>> 缺点：1.空间回收后碎片化 2.清除操作会使程序暂停
> 标记整理，在清除阶段会先执行整理，移动活动对象内存位置，从而使得回收后的空间较连续
>> 优点：空间回收后不会碎片化
>> 缺点：清除操作会使程序暂停
...
* 监控内存情况的方式：
> 浏览器任务管理器
> Timeline 时序图：浏览器性能面板
> 堆快照：浏览器内存面板 snapshot -> detached xxx
